// ─────────────────────────────────────────────────────────────────────────────
// CLIENT-SIDE INSTANT PRICING ENGINE (OFFLINE/ZERO-LATENCY SYNCHRONOUS ROUTINES)
// ─────────────────────────────────────────────────────────────────────────────
...
export function getArea(w: number, h: number, d: number): number {
  const flatW = 2 * w + 2 * d + 4;
  const flatH = h + d / 2 + 8;
  return safeDivide(flatW * flatH, 10000, 0);
}
...
// BAGS CALCULATOR
// ─────────────────────────────────────────────────────────────────────────────
export function clientCalculateBagsPrice(input: any, db: any): any {
  ...
  const categoryDimensions = db.dimensions.filter(
    (dim: any) => dim.categoryId === categoryId && dim.active
  );
  ...
  if (input.paperId) {
    const sp = db.papers.find((p: any) => p.id === input.paperId && p.active);
    if (sp) paperPrice = sp.pricePerSqm;
  } else {
    const matched = db.papers.find(
      (p: any) => p.gsm === input.gsm && p.assignedProducts.includes(categoryId) && p.active
    );
    paperPrice = matched
      ? matched.pricePerSqm
  ...
  let selectedMethod = printingMethods.find((m: any) => m.id === input.method && m.active);
  ...
  if (!selectedMethod || input.method === "auto") {
    ...
    const preferredCandidate = printingMethods.find((m: any) => m.id === defaultId && m.active);
    if (preferredCandidate && isCompatible(preferredCandidate)) {
      ...
      if (compatibleMethods.length > 0) {
        ...
        selectedMethod = preferredCandidate || printingMethods.find((m: any) => m.active && m.allowedCategories?.includes(categoryId));
      }
    }
  }
  ...
  if (input.finishes && Array.isArray(input.finishes)) {
    input.finishes.forEach(fKey => {
      const fin = db.finishes.find((f: any) => f.key === fKey && f.active);
  ...
  if (input.appliedDiscountCode && db.discountCodes) {
    const dcObj = db.discountCodes.find(
      (code: any) => code.code.toUpperCase() === input.appliedDiscountCode!.toUpperCase() && code.active
    );
    if (dcObj) {
...
// BOXES CALCULATOR
// ─────────────────────────────────────────────────────────────────────────────
export function clientCalculateBoxesPrice(input: any, db: any): any {
  const rules = db.pricingRules;
  ...
  if (input.itemId && input.itemId !== "custom") {
    const boxesProduct = db.products.find((p: any) => p.id === "boxes_items");
    const item = boxesProduct?.items.find((it: any) => it.id === input.itemId);
    if (item) {
      itemName = item.name;
  ...
  const boxesCategory = db.categories.find((c: any) => c.id === "boxes");
  ...
  if (input.paperId) {
    selectedPaper = db.papers.find((p: any) => p.id === input.paperId && p.active);
  } else {
    ...
    selectedPaper = db.papers.find((p: any) => p.id === mapped && p.active);
  }
  ...
  if (!selectedMethod || input.method === "auto") {
    ...
    if (preferredCandidate && isCompatible(preferredCandidate)) {
      ...
      if (compatibleMethods.length > 0) {
        ...
        selectedMethod = preferredCandidate || printingMethods.find((m: any) => m.active && m.allowedCategories?.includes("boxes"));
      }
    }
  }
...
// RIBBONS CALCULATOR
// ─────────────────────────────────────────────────────────────────────────────
export function clientCalculateRibbonsPrice(input: any, db: any): any {
  const rules = db.pricingRules;
...
// STICKERS CALCULATOR
// ─────────────────────────────────────────────────────────────────────────────
export function clientCalculateStickersPrice(input: any, db: any): any {
  const rules = db.pricingRules;
...
// ─────────────────────────────────────────────────────────────────────────────
export function clientCalculateGiftCardsPrice(input: any, db: any): any {
  const rules = db.pricingRules;
  ...
  const hasUv = finishesArr.some((f: string) => f.toLowerCase().includes("uv") || f.toLowerCase().includes("լաք"));
  const hasFoil = finishesArr.some((f: string) => f.toLowerCase().includes("foil") || f.toLowerCase().includes("ոսկե") || f.toLowerCase().includes("արծաթ"));
...
// ─────────────────────────────────────────────────────────────────────────────
export function clientCalculateBusinessCardsPrice(input: any, db: any): any {
  const rules = db.pricingRules;
