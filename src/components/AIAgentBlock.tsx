  ...
  return (
    <div className="w-full py-16 px-6 sm:px-10 lg:px-12 max-w-[1240px] mx-auto select-none font-sans" id="ai-agent-meet-block">
      ...
      <div className="text-center mb-10">
        <div className="inline-flex items-center gap-1.5 px-3.5 py-1.5 text-[9px] tracking-[0.25em] uppercase font-mono font-black text-emerald-800 bg-[#f0f2f5] shadow-[3px_3px_6px_#d1d9e6,_-3px_-3px_6px_#FFFFFF] rounded-full mb-4.5">
          <Sparkles size={10} className="text-emerald-700 animate-pulse" />
          <span>Intelligent Assistant</span>
        </div>
        <h2 className="font-sans font-black text-4xl sm:text-5xl text-[#1a1c1d] tracking-tight leading-none uppercase">
          {content.title}
        </h2>
        <p className="text-xs sm:text-sm text-[#1a1c1d]/60 mt-4 max-w-lg mx-auto leading-relaxed">
          {content.subtitle}
        </p>
      </div>
        ...
        {messages.length > 0 && (
          <div className="bg-white/60 backdrop-blur-md rounded-[32px] p-6 mb-8 shadow-[inset_3px_3px_8px_rgba(211,205,191,0.4),_inset_-3.5px_-3.5px_8px_rgba(255,255,255,0.95)] border border-white/85 overflow-hidden animate-fade-in relative">
            <button
              ...
              className="absolute top-4 right-4 text-[10px] font-mono font-bold uppercase tracking-wider text-red-700 hover:text-red-900 transition-all flex items-center gap-1.5 cursor-pointer bg-[#f0f2f5] px-3.5 py-2 rounded-full shadow-[2px_2px_4px_#d1d9e6,_-2px_-2px_4px_#FFFFFF] hover:shadow-[inset_1px_1px_3px_#d1d9e6,_inset_-1px_-1px_3px_#FFFFFF] border-none"
            >
            ...
            <div className="max-h-[380px] overflow-y-auto pr-2 space-y-5 scrollbar-thin">
              {messages.map((msg, index) => (
                ...
                >
                  {msg.role !== "user" && (
                    <div className="w-9 h-9 rounded-full bg-[#f0f2f5] text-[#1a1c1d] flex items-center justify-center shadow-[3px_3px_6px_#d1d9e6,_-3px_-3px_6px_#FFFFFF] shrink-0 border border-white/50">
                  ...
                  <div
                    className={`max-w-[88%] rounded-[24px] px-5 py-3.5 text-sm leading-relaxed ${
                      msg.role === "user"
                        ? "bg-[#1a1c1d] text-white rounded-br-none shadow-[4px_4px_10px_rgba(61,39,27,0.25)]"
                        : "bg-[#f0f2f5] text-[#1a1c1d] border border-white/40 rounded-bl-none shadow-[3px_3px_7px_#d1d9e6,_-3px_-3px_7px_#FFFFFF] markdown-body"
                    }`}
                  >
                    <p className="whitespace-pre-line font-medium leading-[1.6]">{msg.text}</p>
                  </div>
                  {msg.role === "user" && (
                    <div className="w-9 h-9 rounded-full bg-[#f0f2f5] text-[#1a1c1d] flex items-center justify-center font-bold text-xs shadow-[3px_3px_6px_#d1d9e6,_-3px_-3px_6px_#FFFFFF] shrink-0 uppercase border border-white/50">
              ...
              {isLoading && (
                <div className="flex gap-3 justify-start items-center">
                  <div className="w-9 h-9 rounded-full bg-[#f0f2f5] text-[#1a1c1d] flex items-center justify-center shadow-[3px_3px_6px_#d1d9e6,_-3px_-3px_6px_#FFFFFF] shrink-0 border border-white/50 animate-pulse">
                    <Bot size={16} className="text-[#3B52E8]" />
                  </div>
                  <div className="bg-[#f0f2f5] border border-white/40 rounded-[24px] rounded-bl-none px-5 py-3.5 shadow-[3px_3px_7px_#d1d9e6,_-3px_-3px_7px_#FFFFFF] flex items-center gap-2">
                    <Loader2 size={16} className="text-[#3B52E8] animate-spin" />
                    <span className="text-xs font-mono font-bold tracking-wider text-[#1a1c1d]/60 uppercase">
        ...
        <div className="relative rounded-[26px] bg-white/40 backdrop-blur-md p-5 shadow-[inset_4px_4px_10px_rgba(210,203,191,0.45),_inset_-4px_-4px_10px_rgba(255,255,255,0.85)] border border-white/80 mb-8 hover:bg-white/50 transition-all duration-300">
          <div className="w-full flex flex-col sm:flex-row items-center justify-between gap-4">
            ...
            <div className="flex items-center gap-3 shrink-0">
              <button 
                type="button" 
                className="w-10 h-10 bg-[#f0f2f5] text-[#8C8476] hover:text-[#1a1c1d] rounded-full flex items-center justify-center shadow-[3px_3px_6px_#d1d9e6,_-3px_-3px_6px_#FFFFFF] hover:shadow-[inset_2px_2px_4px_#d1d9e6,_inset_-2px_-2px_4px_#FFFFFF] transition-all duration-200 cursor-pointer border border-white/40"
                title="Add Notes"
              >
            ...
            <input
              type="text"
              className="flex-1 w-full bg-[#f0f2f5]/20 py-2.5 px-1 outline-none border-none text-sm sm:text-base font-extrabold font-sans text-[#1a1c1d] placeholder-[#8C8476]/70"
              ...
              <button
                ...
                className={`w-10 h-10 rounded-full flex items-center justify-center transition-all duration-300 border border-white/40 ${
                  query.trim() && !isLoading
                    ? "bg-[#1A3F25] text-white shadow-[4px_4px_8px_#d1d9e6,_-4px_-4px_8px_#FFFFFF] hover:shadow-[inset_3px_3px_5px_rgba(0,0,0,0.3)] cursor-pointer hover:scale-[1.03]"
                    : "bg-[#f0f2f5] text-[#8C8476] cursor-not-allowed opacity-50 shadow-[2px_2px_4px_rgba(180,175,166,0.3)]"
                }`}
              >
                {isLoading ? (
        ...
        <div className="mt-6 space-y-4">
          {content.suggestions.map((suggestion, index) => (
            <button
              ...
              className="w-full flex items-center gap-3.5 px-6 py-4 rounded-[22px] text-left bg-[#f0f2f5] text-[#1a1c1d]/80 hover:text-[#1A3F25] shadow-[6px_6px_12px_#d1d9e6,_-6px_-6px_12px_#FFFFFF] hover:shadow-[inset_3px_3px_6px_#d1d9e6,_inset_-3px_-3px_6px_#FFFFFF] border border-white/30 font-extrabold text-xs sm:text-sm tracking-wide transition-all duration-300 cursor-pointer group"
            >
              <div className="w-8 h-8 rounded-full bg-[#f0f2f5]/60 group-hover:bg-[#f0f2f5]/20 flex items-center justify-center transition-colors shadow-[2px_2px_4px_#d1d9e6,_-2px_-2px_4px_#FFFFFF] group-hover:shadow-[inset_1px_1px_2px_#d1d9e6]">
